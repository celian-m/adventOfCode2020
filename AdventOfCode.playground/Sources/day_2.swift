...
public extension Day2 {
    static func run() {
        let values : [String] = input.components(separatedBy: "\n")
        let pattern : String = #"([0-9]+)\-([0-9]+) ([a-z]): ([a-z]+)"#
        let regexp = try! NSRegularExpression(pattern: pattern, options: [])
        
        let matching = values.map { (line) -> Int in
            if let match = regexp.matches(in: line, options: [], range: NSRange(location: 0, length: line.count)).first {
                let minCountString = Int((line as NSString).substring(with: match.range(at: 1)))!
                let maxCountString = Int((line as NSString).substring(with: match.range(at: 2)))!
                let charString = (line as NSString).substring(with: match.range(at: 3))
                let pwdString = (line as NSString).substring(with: match.range(at: 4))
                
                let occurences = pwdString.map { (char) -> Int in
                    return String(char) == charString ? 1 : 0
                }.reduce(0, +)
                return occurences >= minCountString && occurences <= maxCountString ? 1 : 0
            } else {
                return 0
            }
        }
    ...
    static func run2() {
        ...
        let matching = values.map { (line) -> Int in
            if let match = regexp.matches(in: line, options: [], range: NSRange(location: 0, length: line.count)).first {
                ...
                let isFirstValid = pwdString[minCountString-1] == charString  ? 1 : 0
                let isSecondValid =  pwdString[maxCountString-1] == charString  ? 1 : 0
                
                return isFirstValid+isSecondValid == 1 ? 1 : 0
            } else {
                return 0
            }
        }

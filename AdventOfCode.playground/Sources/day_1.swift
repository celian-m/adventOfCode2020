...
"""
    ...
    public static func run2() {
        ...
        let values = input.split(separator: " ").map {Int($0)!}.sorted(by: {$0 < $1})
        
        
        for  i in 0 ..< (values.count - 3) {
            for j in i ..< (values.count - 2) {
                for k in j ..< (values.count - 1) {
                    let val_i = values[i]
                    let val_j = values[j]
                    let val_k = values[k]
